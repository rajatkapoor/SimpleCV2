...
class Image:
    ...
    **EXAMPLE**
    ...
    def getNumpyT(self):
        return self.getNumpy()[:, :, ::-1].transpose([1, 0, 2])
    ...
    def blit(self, img, pos=None,alpha=None,mask=None,alphaMask=None):
        ...
        **SEE ALSO**
        ...
        if( alpha is not None ):
            ...
            retVal = self.getNumpy()
            xROI, yROI, wROI, hROI = topROI
            npimgCrop = img.getNumpy()[yROI: yROI+hROI, xROI:xROI+wROI]
            maskCrop = mask.getGrayNumpy()[yROI: yROI+hROI, xROI:xROI+wROI]

            xROI, yROI, wROI, hROI = bottomROI
            retValCrop = retVal[yROI: yROI+hROI, xROI:xROI+wROI]

            Image._copyNpwithMask(npimgCrop, retValCrop, maskCrop)
            retVal[yROI: yROI+hROI, xROI:xROI+wROI] = retValCrop

        else:  #vanilla blit
            retVal = self.getNumpy()
            txROI, tyROI, twROI, thROI = topROI
            bxROI, byROI, bwROI, bhROI = bottomROI
            retVal[byROI: byROI+bhROI, bxROI:bxROI+bwROI] = img.getNumpy()[tyROI: tyROI+thROI, txROI:txROI+twROI]

        return Image(retVal)

    @classmethod
    def _copyNpwithMask(self, src, dst, mask):
        """
        **SUMMARY**
        Made this function because there is no longer a function to copy image with 
        a mask in opencv. 
        dst[x,y] = src[x,y] if mask[x,y] != 0

        **PARAMETERS**

        src - src np array to be copied
        dst - dst np array where src is to be copied
        mask - mask np array (single channel)

        **EXAMPLE**
        >>> img = Image("lenna")
        >>> bin = img.binarize().getGrayNumpy()
        >>> inv = img.invert().getNumpy()
        >>> Image._copyNpwithMask(img.getNumpy(), inv, bin)
        >>> Image(inv).show()

        """
        if src.shape[:2] != dst.shape[:2] and src.shape[:2] != mask.shape[:2]:
            warnings.warn("src, dst and mask should have same sizes")
            return None
        if len(mask.shape) != 2:
        ...
        **TODO**
        ...
        if( side == "top" ):
            ...
            if( self.width > image.width ):
                ...
                if( scale ):
                    #scale the other image width to fit
                    topimage = self.resize(w=image.width)                    
                else:
                    topimage = self
            ...
            nH = topimage.height + image.height
            xc = (topimage.width - resized.width)/2
            newCanvas = np.zeros((nH, nW, 3), dtype=np.uint8)
            if xc > 0:
                newCanvas[:topimage.height, :topimage.width] = topimage.getNumpy()
                newCanvas[nH-resized.height:, xc:xc+resized.width] = resized.getNumpy()
            else:
                newCanvas[:topimage.height, abs(xc):abs(xc)+topimage.width] = topimage.getNumpy()
    ...
    def _to32F(self):
        ...
        **SUMMARY**
        ...
        """
        retVal = self.getNumpy().astype(np.float32)
        return retVal
    ...
    def rotate270(self):
        ...
        **EXAMPLE**
        ...
        """
        retVal = cv2.flip(self.getNumpy(), 0) # vertical
        retVal = cv2.transpose(retVal)
        return(Image(retVal, colorSpace=self._colorSpace))
    ...
    def rotate90(self):
        ...
        **EXAMPLE**
        ...
        """
        retVal = cv2.transpose(self.getNumpy())
        retVal = cv2.flip(retVal, 0) # vertical
        return(Image(retVal, colorSpace=self._colorSpace))
    ...
    def rotate180(self):
        ...
        **EXAMPLE**
        ...
        """
        retVal = cv2.flip(self.getNumpy(), 0) #vertical
        retVal = cv2.flip(retVal, 1)#horizontal
        return(Image(retVal, colorSpace=self._colorSpace))
    ...
    def matchSIFTKeyPoints(self, template, quality=200):
        ...
        **SEE ALSO**
        ...
        img = self.transpose().getGrayNumpy()
        template_img = template.transpose().getGrayNumpy()
    ...
    def drawSIFTKeyPointMatch(self, template, distance=200, num=-1, width=1):
        ...
        **SEE ALSO**
        ...
        if template == None:
            return
        resultImg = self.sideBySide(template,scale=False)
        hdif = (self.height-template.height)/2
    ...
    def getFREAKDescriptor(self, flavor="SURF"):
        ...
        **EXAMPLE**
        ...
        self._mKeyPoints, self._mKPDescriptors = extractor.compute(self.getGrayNumpy(), 
                                                                   self._mKeyPoints)
        fs = FeatureSet()
